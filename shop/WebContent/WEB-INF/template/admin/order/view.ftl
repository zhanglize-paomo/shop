	...
	<script type="text/javascript">
		$().ready(function() {
			...
			function acquireLock() {
				$.ajax({
					url: "${base}/admin/order/acquire_lock",
					type: "POST",
					data: {
						orderId: ${order.id}
					},
				...
				<div class="box">
					<div class="box-body">
						[#if !order.hasExpired() && order.status == "pendingReview"]
							<form class="form-horizontal" action="${base}/admin/order/review" method="post">
						...
						[/#if]
							...
							<form id="paymentForm" class="form-horizontal" action="${base}/admin/order/payment" method="post">
								<input name="csrfToken" type="hidden" value="${csrfToken}">
								<input name="orderId" type="hidden" value="${order.id}">
						...
						[#if !order.hasExpired() && order.refundableAmount > 0]
							<form id="refundsForm" class="form-horizontal" action="${base}/admin/order/refunds" method="post">
								<input name="csrfToken" type="hidden" value="${csrfToken}">
								<input name="orderId" type="hidden" value="${order.id}">
						...
						[#if order.shippableQuantity > 0]
							<form id="shippingForm" class="form-horizontal" action="${base}/admin/order/shipping" method="post">
								<input name="csrfToken" type="hidden" value="${csrfToken}">
								<input name="orderId" type="hidden" value="${order.id}">
						...
						[#if order.returnableQuantity > 0]
							<form id="returnsForm" class="form-horizontal" action="${base}/admin/order/returns" method="post">
								<input name="csrfToken" type="hidden" value="${csrfToken}">
								<input name="orderId" type="hidden" value="${order.id}">
						...
						[#if !order.hasExpired() && order.status == "received"]
							<form class="form-horizontal" action="${base}/admin/order/complete" method="post">
								<input name="orderId" type="hidden" value="${order.id}">
						...
						[#if !order.hasExpired() && (order.status == "pendingShipment" || order.status == "shipped" || order.status == "received")]
							<form class="form-horizontal" action="${base}/admin/order/fail" method="post">
								<input name="orderId" type="hidden" value="${order.id}">
