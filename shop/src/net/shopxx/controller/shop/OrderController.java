...
public class OrderController extends BaseController {
	 ...
	 */
	@PostMapping("/create")
	public ResponseEntity<?> create(String cartTag, Long receiverId, Long paymentMethodId, Long shippingMethodId, String code, String invoiceTitle, BigDecimal balance, String memo,  Date shippingDate,@CurrentUser Member currentUser, @CurrentCart Cart currentCart) {
		Map<String, Object> data = new HashMap<>();
		if (currentCart == null || currentCart.isEmpty()) {
			return Results.UNPROCESSABLE_ENTITY;
		}
		...
		Invoice invoice = StringUtils.isNotEmpty(invoiceTitle) ? new Invoice(invoiceTitle, null) : null;
		List<Order> orders = orderService.create(Order.Type.general, currentCart, receiver, paymentMethod, shippingMethod, couponCode, invoice, balance, memo,shippingDate);
	...
	public ResponseEntity<?> create(Long skuId, Integer quantity, Long receiverId, Long paymentMethodId, Long shippingMethodId, BigDecimal balance, String memo, Date shippingDate, @CurrentUser Member currentUser) {
		Map<String, Object> data = new HashMap<>();
		if (quantity == null || quantity < 1) {
			return Results.UNPROCESSABLE_ENTITY;
		}
		...
		cart.setCartItems(cartItems);
		List<String> orderSns = new ArrayList<>();
		List<Order> orders = orderService.create(Order.Type.exchange, cart, receiver, paymentMethod, shippingMethod, null, null, balance, memo,shippingDate);
		for (Order order : orders) {
			if (order != null) {
				orderSns.add(order.getSn());
			}
